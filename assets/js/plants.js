...
(function () {
  ...
  //
  // ?v=2 — cache-bust: these filenames just got reprocessed (every
  // weather variant re-cropped to one shared bounding box per plant,
  // instead of each file's own, to fix cross-weather position drift) —
  ...
  function weatherImages(slug) {
    return {
      clear: 'assets/illustration/plant-' + slug + '-clear.webp?v=2',
      cloudy: 'assets/illustration/plant-' + slug + '-cloudy.webp?v=2',
      fog: 'assets/illustration/plant-' + slug + '-fog.webp?v=2',
      rain: 'assets/illustration/plant-' + slug + '-rain.webp?v=2',
      snow: 'assets/illustration/plant-' + slug + '-snow.webp?v=2',
      storm: 'assets/illustration/plant-' + slug + '-storm.webp?v=2',
    };
  }
  ...
  // line (bottom: 0.90) for every plant, left-to-right matching the real
  // pot order. Expect these to need a round of visual tuning once actually
  ...
  var PLANTS = {
    yellowKalanchoe: {
      ...
      liftAnchor: {centerX: 0.25, bottom: 0.90, height: 0.40},
    },
    violet: {
      ...
      liftAnchor: {centerX: 0.32, bottom: 0.90, height: 0.13},
    },
    orchid: {
      ...
      liftAnchor: {centerX: 0.45, bottom: 0.90, height: 0.32},
    },
    kalanchoe: {
      ...
      liftAnchor: {centerX: 0.60, bottom: 0.90, height: 0.28},
    },
    fiddleFig: {
      ...
      liftAnchor: {centerX: 0.77, bottom: 0.90, height: 0.90},
    },
  };

  // detailAnchor — where/how big each plant is once its card is open — is
  // derived from liftAnchor.height rather than given a flat shared value,
  // so plants keep some size relationship to each other even zoomed in:
  // the fig should still read as bigger than the orchid, which should
  // still read bigger than the violet, matching how they sit at rest,
  // rather than everyone growing to identical size and losing that
  // relationship. Linear (not compressed toward 1, e.g. via sqrt) — a
  // compressed curve kept the smaller plants too close in size to the
  // fig once actually rendered. Linear keeps the gap closer to their
  // resting-size gap while still giving every plant *some* growth via
  // DETAIL_MIN_HEIGHT, and the smaller resulting width (derived from
  // height by aspect ratio) also leaves more clearance before the plant
  // cutout reaches the care-card text on the right.
  var DETAIL_MIN_HEIGHT = 0.38;
  var DETAIL_MAX_HEIGHT = 1.0;
  var maxRestHeight = Object.keys(PLANTS).reduce(function (max, key) {
    return Math.max(max, PLANTS[key].liftAnchor.height);
  }, 0);
  Object.keys(PLANTS).forEach(function (key) {
    var restHeight = PLANTS[key].liftAnchor.height;
    var scale = restHeight / maxRestHeight;
    PLANTS[key].detailAnchor = {
      centerX: 0.22,
      bottom: 0.90,
      height: DETAIL_MIN_HEIGHT + (DETAIL_MAX_HEIGHT - DETAIL_MIN_HEIGHT) * scale,
    };
  });
  ...
  if (!img || !wrap || !info || !infoName || !infoWatered || !infoNeeds || !caption) return;

  var CAPTION_DEFAULT = caption.textContent; // "Kelly's window"
  var CAPTION_BACK = '← Back to ' + CAPTION_DEFAULT;
  ...
  function showPlant(entry) {
    ...
    caption.textContent = CAPTION_BACK;
  ...
  function showWindow() {
    ...
    activeEntry = null;
    caption.textContent = CAPTION_DEFAULT;
  ...
  // showWindow() already no-ops when nothing's open, so this is safe to
  // wire unconditionally rather than only enabling it once a plant opens
  caption.addEventListener('click', showWindow);
